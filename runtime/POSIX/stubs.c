...
#include "posix-runtime/Config/config.h"
...
int __syscall_rt_sigaction(int signum, const struct sigaction *act,
                           struct sigaction *oldact, size_t _something) {
  engine_warning_once("silently ignoring");
  return 0;
}
...
int __socketcall(int type, int *args) {
  engine_warning("ignoring (EAFNOSUPPORT)");
  errno = EAFNOSUPPORT;
  return -1;
}
...
int pipe(int filedes[2]) {
  engine_warning("ignoring (ENFILE)");
  errno = ENFILE;
  return -1;
}
...
clock_t times(struct tms *buf) {
  /* Fake */
  if (!buf)
    engine_warning("returning 0\n");
  else {
    engine_warning("setting all times to 0 and returning 0\n");
    buf->tms_utime = 0;
    buf->tms_stime = 0;
    buf->tms_cutime = 0;
    buf->tms_cstime = 0;
...
int getloadavg(double loadavg[], int nelem) {
  engine_warning("ignoring (-1 result)");
  return -1;
}

pid_t wait(int *status) __attribute__((weak));
pid_t wait(int *status) {
  engine_warning("ignoring (ECHILD)");
  errno = ECHILD;
  return -1;
}
...
int acl_entries(acl_t acl) {
  engine_warning("ignoring (EINVAL)");
  errno = EINVAL;
  return -1;
}
...
acl_t acl_get_file(const char *pathname, acl_type_t type) {
  engine_warning("ignoring (ENONMEM)");
  errno = ENOMEM;
  return NULL;
}
...
int setgid(gid_t gid) {
  engine_warning("silently ignoring (returning 0)");
  return 0;
}
