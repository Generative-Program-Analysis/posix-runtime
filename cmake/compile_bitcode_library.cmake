...
#
#===------------------------------------------------------------------------===#
function(compile_bitcode_library library_name source_files compile_cc_flags compile_cxx_flags engine_prefix opt_suffix)
    # Compile every source file
    set(BC_FILES)
    foreach(source_file ${source_files})
        # Get filename without extension
        get_filename_component(file_name_only "${source_file}" NAME_WE)
        set(bc_file "${CMAKE_CURRENT_BINARY_DIR}/${engine_prefix}_${file_name_only}_${opt_suffix}.bc" )
...
function(add_bitcode_library_targets lib_prefix prefixed_files cc_extra_args cxx_extra_args)
    ...
    foreach(_prefix ${LIB_BC_PREFIX})
        foreach(_suffix ${LIB_BC_SUFFIX})
            set(final_cc_flags ${${_prefix}_LIB_BC_FLAGS_${_suffix}} ${cc_extra_args})
            set(final_cxx_flags ${${_prefix}_LIB_BC_FLAGS_${_suffix}} ${cxx_extra_args})
            compile_bitcode_library("${KLEE_RUNTIME_DIRECTORY}/lib${_prefix}${lib_prefix}${_suffix}.bca" "${prefixed_files}" "${final_cc_flags}" "${final_cxx_flags}" "${_prefix}" "${_suffix}")
            list(APPEND _lib_dependencies "${KLEE_RUNTIME_DIRECTORY}/lib${_prefix}${lib_prefix}${_suffix}.bca")
        endforeach()
    endforeach()
